...
- *They're still challenging.* Even simple games can have a large number of possible board positions; for a game like {{% game 188 %}}Go{{% /game %}} that number[^go] exceeds \\(10^{170}\\). Those board positions are what we call the state space in RL.
- *They're finite.* Even a game like {{% game 1406 %}}Monopoly{{% /game %}}, which can feel like it goes on forever, will eventually end. This means we're dealing with we call finite episodes, so we don't have to worry about infinite horizons.
- *They have a simple built-in reward: win or lose.* One of the strength of RL is that it can learn optimal long term behaviour from short term rewards. In order to formalise this, you usually need to discount rewards from individual actions by an appropriate factor and sum them up over time, which then leads to returns. In board games, there's really only one reward at the very end: win or lose. So that's your return for this episode (i.e., game): 1 if you win, 0 if you lose.
...
|:------|:----:|:----------|
|State|\\(s\\)|The full description of the current state of the game. This could be the positions of the stones in {{% game 188 %}}Go{{% /game %}}, or the way the cards are distributed amongst the players in {{% game 1115 %}}Poker{{% /game %}}, including the order of the cards in the draw pile.|
|Observation|\\(o\\)|The information the player has about the current state of the game. In a game of perfect information like {{% game 188 %}}Go{{% /game %}}, there really is no difference between state and observation, but in games with hidden information, like {{% game 1115 %}}Poker{{% /game %}}, the observation is only a partial description of the state.|
|Action|\\(a\\)|The possible moves a player can make in the current state, e.g., the free (and legal) board positions in {{% game 188 %}}Go{{% /game %}} or *bet*, *call*, *fold* etc in {{% game 1115 %}}Poker{{% /game %}}.|
...
Please remember that none of these are formal definitions, but rather a way to understand the concepts in the context of board games. If you want to dive deeper into RL, I highly recommend the standard textbook by Richard S. Sutton & Andrew G. Barto, which is freely available online: [Reinforcement Learning: An Introduction](http://incompleteideas.net/book/the-book.html). I'll add references to the relevant chapters in that book as we go along. More details on the topics of this article are covered in chapters 1 and 3.
...
[^go]: [Go and mathematics](https://en.wikipedia.org/wiki/Go_and_mathematics).
