...
Let's go back to our "skill chips pot" metaphor. In that view, player *A* would put \\(K p_A\\) chips into the ante, with player *B* contributing \\(K p_B = K (1-p_A)\\). The pot now holds \\(K\\) chips in total as reward for the winner. Because they paid \\(K p\\) chips as buy-in for the game, they've now gained \\(K (1 - p)\\) chips in total, which is exactly our update rule (remember that \\(s=1\\) for the winner).
