...
The other extreme — extremely low RRs — is dominated by games without any rules questions. WEM already mentioned one example ({{% game 318977 %}}MicroMacro{{% /game %}}), but there is one game with an even lower RR after smoothing: {{% game 155250 %}}TseuQuesT{{% /game %}}. Of its 221 threads, not a single one deals with rules questions. What sounds like a great achievement at first sight is actually quite the opposite: almost all that forum chatter is about its unfulfilled crowdfunding campaign — a typical Kickstarter drama. Note: consume RR values with caution.
